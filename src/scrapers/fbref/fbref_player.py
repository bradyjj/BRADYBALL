...
def scrape_fbref_player_data():
    ...
    scrape_player_season_stats(seasons)


def scrape_player_season_stats(seasons):
    for season in tqdm(seasons, desc="Scraping seasons"):
        ...
        for stat_type in FBREF_STAT_CATEGORIES:
            ...
            try:
                ...
                model = FBREF_STAT_CATEGORY_PLAYER_MODELS.get(stat_type)
                table_name = STAT_CATEGORY_PLAYER_TABLE_NAMES.get(stat_type)
                mapping = FBREF_PLAYER_MODELS_MAPPINGS.get(model)
...
def write_failed_rows_to_csv(failed_rows, stat_type):
    ...
    filename = f'players-{stat_type}-failed.csv'
